    ...
    <div className='w-screen h-screen bg-center overflow-x-hidden bg-no-repeat bg-cover bg-[#d1b798]'>
        ...
        <div className='w-[100%] h-fit text-white flex justify-center items-center flex-col mt-10'>
            <div className='w-[90%] h-20 flex justify-between items-center'>
                <h2 className='w-[90%] text-4xl font-semibold'>Events</h2>
                <button className='w-40 h-[60%] text-lg hover:bg-[#8c69488c] bg-[#8c6948b6] rounded-2xl' onClick={()=>setShow(true)}>Add event</button>
            </div>
            ...
            {sites.map((site, key) => (
                <Card name={site.title} img={site.image} id={site.id} event={true}/>
            ))} 
            </div>
            </div>
        </div>
        <AddEvent onClose={()=>setShow(false)} visible={show}/>
    </div>
  )
}
